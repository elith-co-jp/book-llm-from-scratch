...
NVIDIA RTX 6000 Ada Generation を用いて、単一 GPU と DDP（2 GPU 並列）の学習時間を比較しました。
...
両手法とも同じ学習設定（3エポック）で実行しています。2 GPU での並列化により約1.57倍の高速化が得られました。理論上の2倍に達しないのは、GPU 間の勾配集約などの通信オーバーヘッドが存在するためです。GPU 数を増やすほど通信コストの割合が増加するため、実際のスピードアップは線形よりも緩やかになります。

なお、4.3.5 節で紹介した DeepSpeed ZeRO を使用すると、オプティマイザ状態と勾配を GPU 間で分散することでメモリ効率が向上します。今回の実験規模では学習時間に大きな差は見られませんが、より大きなモデルやバッチサイズを扱う場合に ZeRO の恩恵が顕著になります。

### 4.3.7.2 損失の推移

学習の進行に伴う損失の変化を示します。

| ステップ | エポック | train_loss | eval_loss | perplexity |
|---------|---------|------------|-----------|------------|
| 85 | 0.2 | 4.143 | 3.971 | 53.0 |
| 170 | 0.4 | 3.977 | 3.859 | 47.4 |
| 425 | 1.0 | 3.843 | 3.763 | 43.1 |
| 850 | 2.0 | 3.729 | 3.676 | 39.5 |
| 1275 | 3.0 | 3.685 | 3.646 | 38.3 |

eval_loss は 3.97 から 3.65 へと減少し、perplexity は約 53 から約 38 へと改善しました。

### 4.3.7.3 生成サンプル

学習完了後のモデルによる生成例を示します。各プロンプトに対して、モデルが生成した続きの文章です（DDPによる2GPU並列学習の結果）。

**プロンプト1:** 吾輩は猫である。名前はまだ無い。

> 私は猫を飼っているが、猫も犬に劣らず人間も動物であるが、ただの生き物として見れば犬の方が遙かに大きいのだ。しかし人間の生活環境の中でいえば犬の方が遥かに大きな存在であると云うことを忘れてはならないだろう。私は犬と人間の関係というよりも犬と人間が一緒に住んでいる時の社会関係を問題にしてみたいと思う。

**プロンプト2:** 明治時代の

> 文学、美術等に於ては勿論、自然現象も此時代迄とは異る様に成りつつある。日本に於ける芸術、其芸術の社会的地位が著しく低下すると共に、日本の文化が変質するに至ったのであるから、然し其変化は単に偶然に起ったものではない、明治以後の社会構造の変化が、何処で如何なる形に於いて生じたかを知...

**プロンプト3:** 東京の街には

> 私とわたしの母の二人でお花見をした。春が来たと思えば、夏になったりして、春も秋もあったかと思うと、また冬になったりして、寒い日が続いてさびしくてたまらなかった。「おれはあのときなにを思っただろう」と私は考えたことがある。そして今はただ母さんのことを思う...

**プロンプト4:** 先生は言った。「

> あなたがた、おもしろいことを話しているのではない。その人はだれでもできるようなことを言っているのである。それに対しておそろしいことなぞはあるまい。」「そんなことはありません。わたしたちはそれを知っているからこそ、この世においでになる人がいるのだということを知っています。」

生成されたテキストは文法的に正しい日本語で、青空文庫の文体を反映した小説風の文章になっています。ただし、プロンプトの意図を完全に捉えられていない場合もあります。これは事前学習の限界であり、第5章で扱うアラインメント（指示に従うための学習）によって改善されます。
