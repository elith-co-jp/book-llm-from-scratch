...
def build_vocabulary(tokens, max_size=None, min_freq=1, special_tokens=None):
    ...
    # 特殊トークンを追加
    vocabulary = {
        token: idx for idx, token in enumerate(special_tokens + filtered_tokens)
    }
    return vocabulary
...
# =============================================================================
# 3.2.4 BPE (Byte-Pair Encoding) の実装
# =============================================================================
    ...
    def tokenize(self, text):
        ...
        for pair, merge in self.merges.items():
            for split in splits:
                ...
                while i < len(split):
                    is_pair = (
                        i < len(split) - 1  # 現在のインデックスがリストの最後ではない
                        and split[i] == pair[0]  # 現在の文字がペアの最初の文字と一致
                        and split[i + 1] == pair[1]  # 次の文字がペアの2番目の文字と一致
                    )
                    if is_pair:
